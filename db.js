...
async function dbUpdateLogo(squadra_id, logo) {
  await db.from('squadre').update({ logo }).eq('id', squadra_id);
}
...
async function dbSavePartita(p) {
  const { data, error } = await db.from('partite').update({ gol_home: p.gol_home, gol_away: p.gol_away, giocata: p.giocata }).eq('id', p.id).select().single();
...
function subscribeRealtime(callback) {
  db.channel('changes').on('postgres_changes', { event: '*', schema: 'public' }, callback).subscribe();
}
