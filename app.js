    ...
    <div style="padding-bottom:32px;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:22px;">
        <button onclick="selezionaCategoriaPublic(${firstId});setTimeout(()=>{showSection('classifiche',document.querySelector('[data-section=classifiche]'))},150)"
          ...
          onmouseover="this.style.borderColor='var(--blu)';this.style.transform='translateY(-2px)'"
          onmouseout="this.style.borderColor='var(--bordo)';this.style.transform='translateY(0)'">
      ...
      <div style="display:flex;flex-direction:column;gap:8px;">
        ${catFiltrate.map((c,i) => `
          <button onclick="selezionaCategoriaPublic(${c.id})"
            ...
            onmouseover="this.style.borderColor='var(--blu)';this.style.transform='translateY(-1px)'"
            onmouseout="this.style.borderColor='var(--bordo)';this.style.transform='translateY(0)'">
...
function mostraLinkCondivisibile() {
  ...
  navigator.clipboard.writeText(link).then(() => toast('🔗 Link copiato!')).catch(() => prompt('Copia link:', link));
}
  ...
  try {
    ...
    for (const g of gironi) {
      ...
      for (const p of (tuttePartite||[])) {
        ...
        if (newH) sqIds.add(newH); if (newA) sqIds.add(newA);
      }
      if (sqIds.size > 0) {
        ...
        for (const sqId of sqIds) {
          if (!gsIds.has(sqId)) { await db.from('girone_squadre').insert({girone_id: g.id, squadra_id: sqId, posizione: 0}).select(); gsIds.add(sqId); }
        }
      }
    }
    if (risolti>0) {
...
function _resolvePlaceholder(placeholder, classificheGironi, miglioriSecondi=[]) {
  ...
  if (/miglior\s*2[°º]?/i.test(s)) return miglioriSecondi[0]?.sq?.id || null;
  ...
  if (m) {
    const pos = parseInt(m[1]); const tipo = m[2]; const resto = m[3].trim();
    const candidati = [`${tipo} ${resto}`, `${tipo === 'Girone' ? 'Gruppo' : 'Girone'} ${resto}`];
    for (const nomeGirone of candidati) {
      let cl = classificheGironi[nomeGirone];
      if (!cl) { const k = Object.keys(classificheGironi).find(k => k.toLowerCase() === nomeGirone.toLowerCase()); if (k) cl = classificheGironi[k]; }
      if (!cl) { const k = Object.keys(classificheGironi).find(k => k.toLowerCase().endsWith(' ' + resto.toLowerCase())); if (k) cl = classificheGironi[k]; }
  ...
  if (mShort) {
    const pos = parseInt(mShort[1]); const gir = mShort[2].toUpperCase();
    const k = Object.keys(classificheGironi).find(k => k.toUpperCase().endsWith(` ${gir}`) || k.toUpperCase() === `GIRONE ${gir}` || k.toUpperCase() === `GRUPPO ${gir}`);
    if (!k) return null;
...
// ============================================================
//  PUBLIC: CLASSIFICHE
//  Se non ci sono gironi → mostra automaticamente il tabellone
// ============================================================
async function renderClassifiche() {
  ...
  // ── Se non ci sono gironi, reindirizza al tabellone ──
  if (!gironi.length) {
...
async function renderRisultati() {
  ...
  let html = '';
  if (filtroAttivo) {
    html += `<div style="background:linear-gradient(90deg,var(--blu) 0%,var(--blu-lt) 100%);color:white;border-radius:var(--radius);padding:11px 16px;margin-bottom:14px;font-size:14px;font-weight:700;display:flex;align-items:center;gap:10px;">📅 ${STATE.activeGiornata}<span style="font-size:11px;opacity:.7;margin-left:auto;">${tuttePartite.length} partite</span></div>`;
  } else {
    const giornate = STATE._giornateDisponibili || [];
  ...
  if (!STATE._campiGiornate) {
    try { const cg = await dbGetCampiGiornate(STATE.activeTorneo); const campiObj = {}; cg.forEach(c => campiObj[c.giorno] = c); STATE._campiGiornate = campiObj; }
  ...
  const _bannerGiornata = (giorno, isOggi) => {
    ...
    return `<div style="background:${colore};border:1px solid ${bordocolore};border-radius:var(--radius);padding:10px 14px;margin-bottom:10px;">
      <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
        <span style="font-size:13px;font-weight:700;color:${testocolore};white-space:nowrap;">📅 ${giorno}</span>
        ${campo ? `<span style="font-size:12px;color:${isOggi?'rgba(255,255,255,0.85)':'var(--testo-xs)'};flex:1;">📍 <strong>${campo.nome_campo||''}</strong>${campo.nome_campo&&campo.indirizzo?' — ':''}${campo.indirizzo||''}</span>` : `<span style="font-size:11px;color:${isOggi?'rgba(255,255,255,0.45)':'var(--testo-xs)'};font-style:italic;flex:1;">${STATE.isAdmin ? 'Clicca per aggiungere indirizzo' : ''}</span>`}
        ${STATE.isAdmin ? `<button onclick="mostraEditCampoGiornata('${giorno}')" style="background:${isOggi?'rgba(255,255,255,0.2)':'white'};border:1px solid ${isOggi?'rgba(255,255,255,0.3)':'var(--bordo)'};color:${isOggi?'white':'var(--testo-lt)'};border-radius:6px;padding:3px 10px;font-size:11px;cursor:pointer;font-family:inherit;white-space:nowrap;">✏️ ${campo ? 'Modifica' : 'Aggiungi'}</button>` : ''}
      </div>
...
async function renderAdminTornei() {
  ...
  html+=`<div class="section-label">Nuovo torneo</div><div class="card"><div class="form-grid-2"><div class="form-group"><label class="form-label">Nome torneo</label><input class="form-input" id="new-t-nome" placeholder="Torneo 2026"></div><div class="form-group"><label class="form-label">Data</label><input class="form-input" id="new-t-data" placeholder="Dom 22 Mar 2026"></div></div><button class="btn btn-p" onclick="createTorneo()">Crea torneo</button></div>`;
  el.innerHTML=html;
}
...
async function _parseExcelRiga(file, idx) {
  ...
  try {
    ...
    if (btnDiv) { btnDiv.style.display = 'block'; const nome = nomeCatInput?.value || 'categoria'; const btn = btnDiv.querySelector('button'); if (btn) btn.textContent = `✓ Importa "${nome}"`; }
  } catch(e) { if (preview) preview.innerHTML = `<div style="color:var(--rosso);font-size:12px;">❌ Errore: ${e.message}</div>`; }
}

async function _importaRiga(idx) {
  const dati = _fileRighe[idx]; const nomeInput = document.getElementById(`cat-nome-${idx}`); const nomeScritto = nomeInput?.value?.trim();
  ...
  try {
    ...
    if (riga) { riga.style.background = 'var(--verde-bg)'; riga.style.borderColor = 'rgba(22,163,74,0.3)'; const preview = document.getElementById(`cat-preview-${idx}`); if (preview) preview.innerHTML = `<div style="color:var(--verde);font-weight:700;font-size:13px;">✅ Importata!</div>`; if (btn) { btn.disabled = true; btn.textContent = '✅ Importata'; btn.style.background = 'var(--verde)'; } }
    STATE.categorie = await dbGetCategorie(STATE.activeTorneo); renderCatBar();
  } catch(e) { if (btn) { btn.disabled = false; btn.textContent = `✓ Importa "${nomeScritto||'categoria'}"`; } toast('❌ Errore: ' + e.message); console.error(e); }
}

async function rinominaCat(id) {
  const cat = STATE.categorie.find(c => c.id === id); const nomeAttuale = cat ? cat.nome : '';
  const nuovo = prompt('Nuovo nome categoria:', nomeAttuale); if (!nuovo || nuovo.trim() === nomeAttuale) return;
  try { await dbUpdateCategoria(id, { nome: nuovo.trim() }); STATE.categorie = await dbGetCategorie(STATE.activeTorneo); renderCatBar(); renderTorneoBar(); toast('✅ Categoria rinominata!'); await renderAdminSetup(); }
...
async function renderAdminLoghi() {
  ...
  for (const sq of squadre) {
    html+=`<div class="logo-team-row"><div class="logo-upload-btn">${logoHTML(sq,'md')}<div class="logo-plus"><svg width="8" height="8" viewBox="0 0 8 8"><path d="M4 1v6M1 4h6" stroke="white" stroke-width="1.5" stroke-linecap="round"/></svg></div><input type="file" accept="image/*" onchange="uploadLogo(event,${sq.id})"></div><div style="flex:1;"><div style="font-size:14px;font-weight:600;">${sq.nome}</div><div style="font-size:12px;color:#aaa;">${sq.logo?'✅ Logo caricato':'Nessun logo'}</div></div>${sq.logo?`<button class="btn btn-danger btn-sm" onclick="removeLogo(${sq.id})">Rimuovi</button>`:''}</div>`;
  }
  html+='</div>'; el.innerHTML=html;
}

async function caricaLoghiDaCartella(event) {
  const files = Array.from(event.target.files); const log = document.getElementById('loghi-auto-log');
  if (!files.length || !log) return;
  log.style.display = 'block'; log.innerHTML = `📁 ${files.length} file selezionati — abbinamento in corso...<br>`;
  const squadre = await dbGetSquadreFull(STATE.activeTorneo);
  ...
  const norm = (s) => { if (!s) return ''; return s.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[''`]/g,'').replace(PREFISSI,'').replace(SUFFISSI,'').replace(/[^a-z0-9\s]/g,' ').replace(/\s+/g,' ').trim(); };
  ...
  for (const file of files) {
    ...
    const nomeFileNorm = norm(nomeFilePulito); if (!nomeFileNorm) continue;
    ...
    if (bestKey && bestScore >= 0.3) {
      ...
      try { const compressed = await _comprimiImmagine(file, 120, 0.80); for (const sq of targets) await dbUpdateLogo(sq.id, compressed); abbinati++; log.innerHTML += `✓<br>`; }
...
async function comprimiloghiEsistenti() {
  ...
  if (!btn || !log) return;
  btn.disabled = true; btn.textContent = '⏳ Compressione...'; log.style.display = 'block'; log.innerHTML = '📦 Avvio compressione loghi...<br>';
  const squadre = await dbGetSquadreFull(STATE.activeTorneo); const conLogo = squadre.filter(s => s.logo?.startsWith('data:'));
  let compressi = 0, saltati = 0;
  for (const sq of conLogo) {
    const kb = Math.round(sq.logo.length * 0.75 / 1024); if (kb < 15) { saltati++; continue; }
    ...
    try {
      ...
      const newKb = Math.round(compressed.length * 0.75 / 1024); await dbUpdateLogo(sq.id, compressed); compressi++;
      log.innerHTML += `✅ ${newKb}KB (-${Math.round((1-newKb/kb)*100)}%)<br>`;
    } catch(e) { log.innerHTML += `❌ Errore<br>`; }
    log.scrollTop = log.scrollHeight; await new Promise(r => setTimeout(r, 100));
  }
  ...
  return new Promise((resolve, reject) => {
    const processUrl = (src) => {
      ...
      img.onload = () => {
        ...
        canvas.width = w || maxSize; canvas.height = h || maxSize; canvas.getContext('2d').drawImage(img, 0, 0, w, h);
        resolve(canvas.toDataURL('image/jpeg', quality));
      };
      img.onerror = reject; img.src = src;
    };
    if (dataUrl) { processUrl(dataUrl); return; }
    const reader = new FileReader(); reader.onload = (e) => processUrl(e.target.result); reader.onerror = reject; reader.readAsDataURL(file);
  });
}
...
async function renderAdminRisultati() {
  ...
  const campiMap = {}; campiGiornate.forEach(c => campiMap[c.giorno] = c); STATE._campiGiornate = campiMap;
  let tuttePartite = [];
  ...
  for (const p of tuttePartite) {
    ...
    let badge='';
    if (p.giocata) { if(p.gol_home>p.gol_away)badge=`<span class="badge badge-green">${p.home?.nome} vince</span>`; else if(p.gol_home<p.gol_away)badge=`<span class="badge badge-green">${p.away?.nome} vince</span>`; else badge=`<span class="badge badge-blue">Pareggio</span>`; }
    const orInfo=`<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-bottom:4px;">${p.orario?`<span style="font-size:11px;font-weight:700;color:#E85C00;">🕐 ${p.orario}</span>`:''}${p.campo?`<span style="font-size:11px;color:#888;">📍 ${p.campo}</span>`:''}  <span style="font-size:11px;color:#bbb;">${p._girone}</span>${p.inserito_da?`<span style="font-size:10px;color:#888;margin-left:auto;">✏️ ${p.inserito_da}</span>`:''}</div>`;
    html+=`<div class="admin-match"><div class="admin-match-header">${orInfo}<div style="display:flex;align-items:center;gap:6px;width:100%;flex-wrap:wrap;"><div class="admin-team-name">${logoHTML(p.home,'sm')}<span>${p.home?.nome||'?'}</span></div><input class="score-input" type="number" min="0" max="30" value="${p.giocata?p.gol_home:''}" placeholder="—" id="sh_${p.id}"><span class="score-dash">—</span><input class="score-input" type="number" min="0" max="30" value="${p.giocata?p.gol_away:''}" placeholder="—" id="sa_${p.id}"><div class="admin-team-name right"><span>${p.away?.nome||'?'}</span>${logoHTML(p.away,'sm')}</div><div class="match-actions"><button class="btn btn-p btn-sm" onclick="saveRisultato(${p.id},${p._gironeId})">✓ Conferma</button>${badge}${p.giocata?`<button class="btn btn-accent btn-sm" onclick="toggleScorers('${key}')">${open?'Chiudi':'+ Marcatori'}</button>`:''}</div></div></div>`;
    if (open&&p.giocata) {
...
async function saveRisultato(partita_id, girone_id) {
  ...
  try {
    ...
    if (result) { toast('✓ Salvato!'); await renderAdminRisultati(); const {data:gironeRow}=await db.from('gironi').select('categoria_id').eq('id',girone_id).single(); if (gironeRow?.categoria_id) await verificaEGeneraTriangolari(gironeRow.categoria_id); }
...
async function saveMarcatori(partita_id, girone_id) {
  ...
  for (let i=0;i<(partita.marcatori||[]).length;i++) {
    const sqEl=document.getElementById(`msq_${partita_id}_${i}`); const nmEl=document.getElementById(`mnm_${partita_id}_${i}`); const mnEl=document.getElementById(`mmin_${partita_id}_${i}`);
...
async function renderAdminKnockout() {
  ...
  if (!STATE.activeCat) { el.innerHTML='<div class="empty-state">Nessuna categoria.</div>'; return; }
  await verificaEGeneraTriangolari(STATE.activeCat);
  const ko=await dbGetKnockout(STATE.activeCat); const squadre=await dbGetSquadre(STATE.activeTorneo);
  const sqMap={}; squadre.forEach(s=>sqMap[s.id]=s);
  const pending=ko.filter(k=>(!k.home_id||!k.away_id)&&(k.note_home||k.note_away));
  let html='';
  if (pending.length) { html+=`<div class="card" style="border-left:4px solid #e67e22;margin-bottom:14px;"><div style="font-size:13px;font-weight:700;color:#e67e22;">⏳ ${pending.length} accoppiamenti in attesa dei gironi</div><button class="btn btn-p btn-sm" style="margin-top:10px;" onclick="risolviManuale()">🔄 Risolvi ora</button></div>`; }
  ...
  const ROUND_COLORS={'PLATINO':'#FFD700','GOLD':'#FFA500','SILVER':'#C0C0C0','BRONZO':'#CD7F32','WHITE':'#B0BEC5'};
  for (const [rname,rmatch] of Object.entries(rounds)) {
    const rkey=Object.keys(ROUND_COLORS).find(k=>rname.toUpperCase().includes(k)); const color=ROUND_COLORS[rkey]||'#E85C00';
    ...
    for (const m of rmatch) {
      const hm=m.home_id?sqMap[m.home_id]:null; const am=m.away_id?sqMap[m.away_id]:null;
      const hmNome=hm?hm.nome:`<em style="color:#e67e22;">${m.note_home||'?'}</em>`; const amNome=am?am.nome:`<em style="color:#e67e22;">${m.note_away||'?'}</em>`;
      const risolto=!!(hm&&am);
      let badge=''; if (m.giocata) { if(m.gol_home>m.gol_away)badge=`<span class="badge badge-green">${hm?.nome} vince</span>`; else if(m.gol_home<m.gol_away)badge=`<span class="badge badge-green">${am?.nome} vince</span>`; else badge=`<span class="badge badge-blue">Pareggio</span>`; }
      ...
      html+=`<div class="admin-match"><div class="admin-match-header">${orInfo}<div style="display:flex;align-items:center;gap:6px;width:100%;flex-wrap:wrap;"><div class="admin-team-name">${logoHTML(hm,'sm')}<span>${hmNome}</span></div><input class="score-input" type="number" min="0" value="${m.giocata?m.gol_home:''}" placeholder="—" id="ksh_${m.id}" ${!risolto?'disabled':''}><span class="score-dash">—</span><input class="score-input" type="number" min="0" value="${m.giocata?m.gol_away:''}" placeholder="—" id="ksa_${m.id}" ${!risolto?'disabled':''}><div class="admin-team-name right"><span>${amNome}</span>${logoHTML(am,'sm')}</div><div class="match-actions">${risolto?`<button class="btn btn-p btn-sm" onclick="saveKO(${m.id})">✓ Conferma</button>`:''}${badge}${!risolto?`<span style="font-size:11px;color:#e67e22;">⏳ In attesa gironi</span>`:''}</div></div></div></div>`;
    }
    html+=`</div>`;
  }
  el.innerHTML=html;
...
async function risolviManuale() {
  ...
  let risolti=0;
  for (const match of (matches||[])) {
    const newH=_resolvePlaceholder(match.note_home,classificheGironi); const newA=_resolvePlaceholder(match.note_away,classificheGironi);
    if ((newH&&newH!==match.home_id)||(newA&&newA!==match.away_id)) { const upd={}; if(newH)upd.home_id=newH; if(newA)upd.away_id=newA; await db.from('knockout').update(upd).eq('id',match.id); risolti++; }
  }
  if (risolti>0) { _mostraNotificaTriangolari(); await renderAdminKnockout(); } else toast('ℹ️ Nessun accoppiamento da aggiornare');
}
...
function checkPw() {
  ...
  _saveLogin(username,password); document.getElementById('admin-modal').style.display='none';
...
function enterAdmin(user) {
  ...
  document.getElementById('pub-nav').style.display='none'; document.getElementById('admin-nav').style.display='flex';
...
function _addSimBtn() {
  ...
  const btn = document.createElement('button'); btn.id = 'sim-toggle-btn'; btn.textContent = '🎮'; btn.title = 'Modalità Simulazione';
  ...
  btn.onmouseover = () => btn.style.background = 'rgba(245,158,11,0.25)'; btn.onmouseout = () => btn.style.background = 'rgba(245,158,11,0.12)';
  btn.onclick = toggleSimulazione;
  const headerRight = document.querySelector('.header-right'); if (headerRight) headerRight.insertBefore(btn, headerRight.firstChild);
}
...
function _mostraNavArbitro() {
  ...
  const btnRis=nav.querySelector('[data-section="a-risultati"]'); if (btnRis) { btnRis.style.display=''; btnRis.classList.add('active'); }
}
...
function exitAdmin() {
  ...
  document.getElementById('pub-nav').style.display='flex'; document.getElementById('admin-nav').style.display='none';
  ...
  _simUnlocked = false; STATE.currentSection='classifiche';
  document.querySelectorAll('.nav-btn:not(.nav-exit)').forEach(b=>b.classList.remove('active'));
...
function toast(msg) {
  ...
  t.textContent=msg; t.style.display='block'; clearTimeout(t._timer); t._timer=setTimeout(()=>t.style.display='none',2500);
}
...
function openTV() {
  ...
  _tvState.active = true; _tvState.catIndex = 0; tv.classList.add('active');
  _tvRenderCatBar(); _tvShowCat(0); _tvStartClock(); _tvStartAutoscroll();
}

function closeTV() {
  _tvState.active = false; const tv = document.getElementById('tv-mode'); if (tv) tv.classList.remove('active');
...
function _tvRenderCatBar() {
  ...
  bar.innerHTML = STATE.categorie.map((c, i) => `<button class="tv-cat-btn ${i === _tvState.catIndex ? 'active' : ''}" onclick="_tvShowCat(${i})">${_abbreviaNomeCat(c.nome)}</button>`).join('');
}

async function _tvShowCat(idx) {
  _tvState.catIndex = idx; _tvRenderCatBar();
  const cats = _tvCategorieFiltrate(); if (!cats[idx]) return;
  const oggi = _tvGetOggi(); const content = document.getElementById('tv-content'); if (!content) return;
  content.innerHTML = '<div style="color:rgba(255,255,255,0.3);padding:40px;text-align:center;">Caricamento...</div>';
  try {
    if (_tvState.section === 'classifiche') await _tvRenderClassifiche(cats[idx], oggi, content);
    else await _tvRenderRisultati(cats[idx], oggi, content);
    content.scrollTop = 0;
  } catch(e) { console.error(e); }
}
...
async function _tvRenderClassifiche(cat, oggi, container) {
  const gironi = await getGironiWithData(cat.id); let html = '';
  ...
  for (const g of gironi) {
    ...
    cl.forEach((row, idx) => { const q = idx < (cat.qualificate || 1); html += `<tr class="${q?'qualifies':''}"><td><span class="${q?'q-dot':'nq-dot'}"></span></td><td>${logoHTML(row.sq,'sm')}</td><td>${row.sq.nome}</td><td style="color:rgba(255,255,255,0.5)">${row.g}</td><td class="tv-pts">${row.pts}</td></tr>`; });
...
async function _tvRenderRisultati(cat, oggi, container) {
  const gironi = await getGironiWithData(cat.id); let tuttePartite = [];
  for (const g of gironi) { for (const p of g.partite) tuttePartite.push({ ...p, _girone: g.nome }); }
  if (oggi) { const filtrateOggi = tuttePartite.filter(p => p.giorno === oggi); if (filtrateOggi.length) tuttePartite = filtrateOggi; }
  tuttePartite.sort((a, b) => _orarioToMinuti(a.orario) - _orarioToMinuti(b.orario));
  ...
  if (giocate.length) html += `<div style="font-size:11px;font-weight:700;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:.08em;margin-bottom:12px;">✅ Risultati</div><div class="tv-results-grid">${giocate.map(p=>matchCard(p,false)).join('')}</div>`;
  if (daFare.length) html += `<div style="font-size:11px;font-weight:700;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:.08em;margin:20px 0 12px;">🕐 Programma</div><div class="tv-results-grid">${daFare.map(p=>matchCard(p,true)).join('')}</div>`;
...
function _tvSwitchSection(section, btn) {
  _tvState.section = section; document.querySelectorAll('.tv-nav-btn').forEach(b => b.classList.remove('active'));
  if (btn) btn.classList.add('active'); _tvShowCat(_tvState.catIndex);
}

function _tvStartClock() {
  const update = () => {
    const now = new Date(); const el = document.getElementById('tv-clock'); if (el) el.textContent = now.toLocaleTimeString('it-IT', { hour:'2-digit', minute:'2-digit' });
...
function _tvStartAutoscroll() {
  ...
  _tvState.scrollTimer = setInterval(() => {
    elapsed += TICK;
    const pct = (elapsed / SCROLL_INTERVAL) * 100; const prog = document.getElementById('tv-scroll-progress'); if (prog) prog.style.width = Math.min(pct, 100) + '%';
    const content = document.getElementById('tv-content');
    if (content) { content.scrollTop += 2; if (content.scrollTop + content.clientHeight >= content.scrollHeight - 10) elapsed = SCROLL_INTERVAL; }
    if (elapsed >= SCROLL_INTERVAL) { elapsed = 0; if (prog) prog.style.width = '0%'; const cats = _tvCategorieFiltrate(); _tvShowCat((_tvState.catIndex + 1) % cats.length); }
  }, TICK);
}
...
function toggleSimulazione() {
  ...
  const pwd = prompt('🔐 Inserisci la password per la modalità simulazione:'); if (!pwd) return;
...
function _renderSimPanel() {
  ...
  panel.innerHTML = `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;"><div style="color:#f59e0b;font-size:14px;font-weight:800;letter-spacing:.04em;">🎮 SIMULAZIONE</div><button onclick="document.getElementById('sim-panel').remove()" style="background:rgba(239,68,68,0.15);border:1px solid rgba(239,68,68,0.3);color:#ef4444;border-radius:6px;padding:3px 8px;cursor:pointer;font-size:12px;font-family:inherit;">✕</button></div>
...
function _golCasuale() { const r = Math.random(); if(r<0.15)return 0; if(r<0.35)return 1; if(r<0.55)return 2; if(r<0.70)return 3; if(r<0.82)return 4; if(r<0.91)return 5; if(r<0.96)return 6; return 7; }
...
async function simulaRisultati() {
  ...
  try {
    const catId = STATE.activeCat || STATE.categorie[0]?.id; let totale = 0;
    for (let pass = 1; pass <= 6; pass++) {
      ...
      for (const g of gironi) {
        const daGiocare = g.partite.filter(p => !p.giocata && p.home_id && p.away_id); if (!daGiocare.length) continue;
        _simLog(`Pass ${pass} — ${g.nome}: ${daGiocare.length} partite`);
        for (const p of daGiocare) { const gh = _golCasuale(), ga = _golCasuale(); await dbSavePartita({ id:p.id, girone_id:p.girone_id, gol_home:gh, gol_away:ga, giocata:true, inserito_da:'🤖 Simulazione' }); nuovi++; totale++; }
      }
      if (nuovi === 0) { _simLog(`✓ Completato in ${pass} passaggi!`); break; }
    }
    if (catId) await verificaEGeneraTriangolari(catId);
    _simLog(`✅ ${totale} risultati simulati!`); toast(`✅ ${totale} risultati simulati!`); await renderCurrentSection();
...
async function simulaRisultatiGirone() {
  ...
  try {
    ...
    for (const g of gironi) {
      ...
      for (const p of daGiocare) { await dbSavePartita({ id: p.id, girone_id: p.girone_id, gol_home: _golCasuale(), gol_away: _golCasuale(), giocata: true, inserito_da: '🤖 Simulazione' }); totale++; }
      break;
    }
    _simLog(`✅ ${totale} risultati simulati!`); toast(`✅ ${totale} risultati simulati!`);
    if (STATE.activeCat) await verificaEGeneraTriangolari(STATE.activeCat); await renderCurrentSection();
...
async function resetRisultati() {
  ...
  const log = document.getElementById('sim-log'); if (log) log.innerHTML = ''; _simLog('⏳ Reset in corso...');
  try {
    ...
    for (const catId of cats) {
      ...
      for (const g of gironi) { await db.from('partite').update({ gol_home: 0, gol_away: 0, giocata: false, inserito_da: null }).eq('girone_id', g.id); totale++; }
      ...
      if (gironiIds.length) { const { data: partite } = await db.from('partite').select('id').in('girone_id', gironiIds); if (partite?.length) await db.from('marcatori').delete().in('partita_id', partite.map(p => p.id)); }
    }
...
function mostraEditCampoGiornata(giorno) {
  ...
  div.innerHTML = `<div style="background:white;border:1px solid var(--bordo);border-radius:8px;padding:12px;display:flex;flex-direction:column;gap:8px;"><div style="font-size:11px;font-weight:700;color:var(--testo-xs);text-transform:uppercase;">📍 Luogo partite — ${giorno}</div><div style="display:flex;gap:8px;flex-wrap:wrap;"><input id="edit-nome-${keyId}" class="form-input" style="flex:1;min-width:120px;font-size:13px;" placeholder="Nome campo" value="${esistente.nome_campo||''}"><input id="edit-addr-${keyId}" class="form-input" style="flex:2;min-width:200px;font-size:13px;" placeholder="Indirizzo" value="${esistente.indirizzo||''}"><button onclick="salvaCampoGiornata('${giorno}')" style="background:var(--blu);color:white;border:none;border-radius:8px;padding:8px 14px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;white-space:nowrap;">✓ Salva</button></div></div>`;
