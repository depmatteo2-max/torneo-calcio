...
function _saveSavedTorneo(id) {
  try {
    localStorage.setItem('spe_torneo', String(id));
  } catch (e) {}
}

function _loadSavedTorneo() {
  try {
    const v = localStorage.getItem('spe_torneo');
...
function renderTorneoBar() {
  ...
  bar.innerHTML = `<div class="torneo-bar-inner">
    ...
    <button onclick="cambiaTorneo()"
      style="background:#f0f4f8;border:1px solid #dde3ea;border-radius:8px;padding:5px 12px;font-size:12px;font-weight:600;color:#2e86c1;cursor:pointer;">
      🔄 Cambia torneo
    </button>
  </div>`;
}
...
function _risolviGruppi(lista, giocate) {
  ...
  while (i < lista.length) {
    ...
    gruppo.forEach(s => {
      si[s.sq.id] = { pt: 0, dr: 0, gf: 0 };
    });

    pInt.forEach(p => {
      const h = si[p.home_id];
      const a = si[p.away_id];
      h.dr += p.gol_home - p.gol_away;
      a.dr += p.gol_away - p.gol_home;
      h.gf += p.gol_home;
      a.gf += p.gol_away;
...
function _mostraNotificaTriangolari() {
  ...
  div.innerHTML = `
    <div style="position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:#1e8449;color:white;padding:14px 24px;border-radius:12px;font-size:14px;font-weight:700;z-index:9999;box-shadow:0 4px 20px rgba(0,0,0,0.3);display:flex;align-items:center;gap:10px;max-width:90vw;">
      ...
      <button onclick="document.getElementById('notifica-triangolari').remove()"
        style="background:rgba(255,255,255,0.2);border:none;color:white;padding:2px 8px;border-radius:6px;cursor:pointer;">✕</button>
    </div>`;
...
async function renderAdminSetup() {
  ...
  if (!STATE.activeTorneo) {
    el.innerHTML = '<div class="empty-state">Crea prima un torneo.</div>';
    return;
  }
  ...
  if (tutteSquadre.length) {
    ...
    tutteSquadre.forEach(sq => {
      html += `<span style="display:inline-flex;align-items:center;gap:5px;background:#f0f6ff;border:1px solid #c5ddf5;border-radius:99px;padding:3px 10px;font-size:12px;color:#185FA5;">
  ...
  if (!STATE.categorie.length) {
    html += `<div style="color:#aaa;font-size:13px;padding:8px 0 12px;">Nessuna categoria. Importa da Excel o aggiungine una manualmente.</div>`;
  }
  ...
  for (const cat of STATE.categorie) {
    ...
    for (const g of gironi) {
      ...
      totP += pp.length;
      totG += pp.filter(p => p.giocata).length;
    }
    ...
    for (const g of gironi) {
      ...
      members.forEach(m => {
        html += `<span style="display:inline-flex;align-items:center;gap:4px;background:#f5f5f5;border-radius:99px;padding:2px 8px;font-size:12px;">
    ...
    if (ko.length) {
      ...
      Object.entries(rounds).forEach(([rname, rmatches]) => {
        ...
        html += `<div style="display:inline-flex;align-items:center;gap:6px;background:#fafafa;border:1px solid #eee;border-left:3px solid ${color};border-radius:6px;padding:4px 10px;margin:3px;font-size:12px;">
          <strong>${rname}</strong>
...
async function saveRisultato(partita_id, girone_id) {
  ...
  try {
    const result = await dbSavePartita({
      id: partita_id,
      girone_id,
      gol_home: parseInt(sh),
      gol_away: parseInt(sa),
      giocata: true
    });
...
function addMarcatore(pid) {
  if (!tempMarcatori[pid]) tempMarcatori[pid] = [];
  tempMarcatori[pid].push({});
  renderAdminRisultati();
}

function removeMarcatore(pid, idx) {
  if (!tempMarcatori[pid]) tempMarcatori[pid] = [];
  tempMarcatori[pid].splice(idx, 1);
  renderAdminRisultati();
}
...
async function saveMarcatori(partita_id, girone_id) {
  ...
  for (let i = 0; i < (partita.marcatori || []).length; i++) {
    ...
    if (sqEl && nmEl && nmEl.value.trim()) {
      all.push({
        squadra_id: parseInt(sqEl.value),
...
async function risolviManuale() {
  ...
  if (!gironi || !gironi.length) {
    toast('Nessun girone trovato');
    return;
  }
  ...
  if (risolti > 0) {
    ...
    toast('ℹ️ Nessun accoppiamento da aggiornare');
  }
}
...
function checkPw() {
  ...
  if (!userEl || !pwEl) {
    console.error('Campi login non trovati');
    if (errEl) errEl.textContent = 'Errore login: campi non trovati';
    return;
  }
  ...
  if (!user) {
    if (errEl) errEl.textContent = 'Username o password errati';
    pwEl.select();
    return;
  }
...
function _mostraNavArbitro() {
  ...
  if (btnRis) {
    btnRis.style.display = '';
    btnRis.classList.add('active');
  }
}
...
function _saveLogin(username, password) {
  try {
    localStorage.setItem('spe_login', JSON.stringify({ username, password }));
  } catch (e) {}
}

function _loadSavedLogin() {
  try {
    const v = localStorage.getItem('spe_login');
...
function _clearLogin() {
  try {
    localStorage.removeItem('spe_login');
  } catch (e) {}
}
