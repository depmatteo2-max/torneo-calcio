...
function mostraSelezioneTeorneo() {
  ...
  if (typeof getLogo === 'function') {
    ...
    if (logoData) { const img = document.getElementById('sel-logo'); if (img) img.src = logoData; }
  }
}
...
async function renderClassifiche() {
  ...
  for (const g of gironi) {
    ...
    html+=`<div class="card"><div class="card-title">${g.nome}<span class="badge badge-gray">${played}/${total} partite</span></div>
    ...
    if (giocate.length) {
      ...
      for (const p of giocate) {
        ...
        html+=`<div class="match-result">
          ...
          <div style="display:flex;flex-direction:column;align-items:center;">${orario}<div class="match-score">${p.gol_home} — ${p.gol_away}</div></div>
    ...
    for (const [rname,rmatch] of Object.entries(rounds)) {
      ...
      for (const m of rmatch) {
        ...
        h+=`<div class="match-result ${isPending?'pending-match':''}">
          ...
          <div style="display:flex;flex-direction:column;align-items:center;">${orario}<div class="match-score ${!m.giocata?'pending':''}">${m.giocata?m.gol_home+' — '+m.gol_away:'vs'}</div></div>
...
async function renderAdminSetup() {
  ...
  let html=`<div style="background:#fff3e0;border-radius:10px;padding:12px 16px;margin-bottom:14px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
...
async function renderAdminKnockout() {
  ...
  if (pending.length) {
    html+=`<div class="card" style="border-left:4px solid #e67e22;margin-bottom:14px;">
      ...
      <div style="font-size:12px;color:#888;margin-top:4px;">Appena inserisci l'ultimo risultato del girone, le squadre vengono assegnate automaticamente.</div>
...
// ============================================================
//  AUTH - CORRETTO: usa id="admin-user" e id="admin-pw"
// ============================================================
...
function checkPw() {
  ...
  const pwEl=document.getElementById('admin-pw');
  const errEl=document.getElementById('pw-error');
  if (!userEl||!pwEl) { console.error('Campi login non trovati'); if(errEl)errEl.textContent='Errore tecnico'; return; }
...
function enterAdmin(user) {
  ...
  if (user.ruolo==='arbitro') {
    ...
    renderCatBar(); renderAdminRisultati();
  } else {
    document.querySelectorAll('.nav-btn:not(.nav-exit)').forEach(b=>b.classList.remove('active'));
    ...
    document.getElementById('cat-bar').style.display='none';
    STATE.currentSection='a-tornei'; renderAdminTornei();
  }
}
...
function _saveLogin(username,password) { try { localStorage.setItem('spe_login',JSON.stringify({username,password})); } catch(e){} }
