...
async function renderClassifiche() {
  ...
  el.innerHTML = html;
  const sel = document.getElementById('manual-girone');
  if (sel) {
    let opts = '';
    for (const cat of STATE.categorie) {
      const gironi = await dbGetGironi(cat.id);
      for (const g of gironi) opts += `<option value="${g.id}">${cat.nome} · ${g.nome}</option>`;
    }
    sel.innerHTML = opts;
  }
}

async function addPartitaManuale() {
  const girone_id = parseInt(document.getElementById('manual-girone').value, 10);
  const ids = (document.getElementById('manual-team-ids').value || '').split(',').map(x=>parseInt(x.trim(),10)).filter(Boolean);
  const orario = document.getElementById('manual-orario').value.trim();
  const campo = document.getElementById('manual-campo').value.trim();
  if (ids.length !== 2) { alert('Inserisci due ID squadra separati da virgola'); return; }
  await dbCreatePartita({ girone_id, home_id: ids[0], away_id: ids[1], orario, campo, giornata: 1, ordine: 999, giocata: false, manuale: true });
  toast('Partita manuale aggiunta');
  await renderAdminRisultati();
}


function partitaMetaHTML(p) {
  const bits = [];
  if (p.giornata) bits.push(`Giornata ${p.giornata}`);
  if (p.orario) bits.push(p.orario);
  if (p.campo) bits.push(p.campo);
  if (!bits.length) return '';
  return `<div style="font-size:12px;color:#7a7a7a;margin:4px 0 8px 0;">${bits.join(' · ')}</div>`;
}

function parsePriorityLines(text, allSquadre) {
  const map = {};
  const rows = text.split('\n').map(x=>x.trim()).filter(Boolean);
  for (const row of rows) {
    const [nameRaw, priRaw] = row.split(':');
    const sq = allSquadre.find(s => s.nome.toLowerCase() === (nameRaw || '').trim().toLowerCase());
    if (sq) map[sq.id] = parseInt(priRaw || '1', 10) || 1;
  }
  return map;
}

function parseSlotLines(text) {
  return text.split('\n').map(x=>x.trim()).filter(Boolean).map((row, idx) => {
    const [orario='', campo=''] = row.split('|').map(x=>x.trim());
    return { orario, campo, giornata: idx + 1 };
  });
}
...
Girone C: Atalanta, Lazio, Bologna, Genoa"></textarea></div>
    <div class="form-group"><label class="form-label">Priorità squadre (una per riga: Nome:priorità)</label>
      <textarea class="form-input" id="new-cat-priority" rows="3" placeholder="Juventus:3\nMilan:2"></textarea></div>
    <div class="form-group"><label class="form-label">Slot orari/campi (una riga: orario|campo)</label>
      <textarea class="form-input" id="new-cat-slots" rows="3" placeholder="09:00|Campo 1\n09:30|Campo 2"></textarea></div>
    <div class="form-grid-2">
  ...
  <div class="section-label">Partita manuale</div>
  <div class="card">
    <div class="form-grid-2">
      <div class="form-group"><label class="form-label">Girone</label><select class="form-input" id="manual-girone"></select></div>
      <div class="form-group"><label class="form-label">Casa ID / Trasferta ID</label><input class="form-input" id="manual-team-ids" placeholder="1,2"></div>
      <div class="form-group"><label class="form-label">Orario</label><input class="form-input" id="manual-orario" placeholder="09:30"></div>
      <div class="form-group"><label class="form-label">Campo</label><input class="form-input" id="manual-campo" placeholder="Campo 1"></div>
    </div>
    <button class="btn btn-p" onclick="addPartitaManuale()">Aggiungi partita manuale</button>
    <div style="font-size:12px;color:#777;margin-top:8px;">Usa gli ID delle squadre presenti nel torneo. Se vuoi, poi te li faccio vedere anche in elenco.</div>
  </div>`;
...
async function addCategoria() {
  ...
  const priorityText = document.getElementById('new-cat-priority').value.trim();
  const slotsText = document.getElementById('new-cat-slots').value.trim();
  ...
  for (let gi = 0; gi < lines.length; gi++) {
    ...
    const squadra_ids = [];
    const tutte = await dbGetSquadre();
    for (const tn of teamNames) {
      let sq = tutte.find(s => s.nome.toLowerCase() === tn.toLowerCase());
      if (!sq) sq = await dbSaveSquadra({ nome: tn });
      squadra_ids.push(sq.id);
    }
    await dbSetGironeSquadre(girone.id, squadra_ids);
    const priorities = parsePriorityLines(priorityText, await dbGetSquadre());
    const slots = parseSlotLines(slotsText);
    await dbGeneraPartite(girone.id, squadra_ids, { priorities, slots });
  }
  renderCatBar();
    ...
    for (const p of g.partite) {
      ...
      html += `<div class="admin-match">
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px;">
          <input class="form-input" style="max-width:120px;" id="or_${p.id}" placeholder="Orario" value="${p.orario || ''}">
          <input class="form-input" style="max-width:120px;" id="ca_${p.id}" placeholder="Campo" value="${p.campo || ''}">
          <input class="form-input" style="max-width:90px;" id="gi_${p.id}" placeholder="Giornata" value="${p.giornata || ''}">
          <input class="form-input" style="max-width:90px;" id="ord_${p.id}" placeholder="Ordine" value="${p.ordine || ''}">
          ${p.manuale ? '<span class="badge badge-blue">Manuale</span>' : ''}
        </div>
...
async function saveRisultato(partita_id, girone_id) {
  ...
  const orario = document.getElementById('or_' + partita_id)?.value?.trim() || '';
  const campo = document.getElementById('ca_' + partita_id)?.value?.trim() || '';
  const giornata = parseInt(document.getElementById('gi_' + partita_id)?.value || '0', 10) || null;
  const ordine = parseInt(document.getElementById('ord_' + partita_id)?.value || '0', 10) || null;
  if (sh === '' || sa === '') return;
  await dbSavePartita({ id: partita_id, girone_id, gol_home: parseInt(sh), gol_away: parseInt(sa), giocata: true, orario, campo, giornata, ordine });
