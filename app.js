...
async function verificaEGeneraTriangolari(categoriaId) {
 ...
 // Mappa per round_name pulito (es. "⚔️ SEMIFINALE 01 — Semifinale 1" → "SEMIFINALE 01")
 ...
 girone_id: g.id, squadra_id: sqId, posizione: 0
 }).select();
 gsIds.add(sqId);
 }
 }
...
function _resolvePlaceholder(placeholder, classificheGironi, miglioriSecondi=[], risultatiKnockout={}) {
 ...
 if (mSemVP) {
 const tipo = mSemVP[1].toLowerCase(); // 'vincente' o 'perdente'
 const semNum = mSemVP[2].padStart(2,'0');
 const semKey = `SEMIFINALE ${semNum}`;
 const sem = risultatiKnockout[semKey];
 if (!sem || !sem.giocata) return null;
 if (tipo === 'vincente') {
 if (sem.gol_home > sem.gol_away) return sem.home_id;
 if (sem.gol_away > sem.gol_home) return sem.away_id;
 return sem.home_id;
 } else {
 if (sem.gol_home < sem.gol_away) return sem.home_id;
 if (sem.gol_away < sem.gol_home) return sem.away_id;
...
async function saveKO(match_id) {
 ...
 // Risolvi automaticamente gli accoppiamenti successivi (Vincente/Perdente)
...
async function risolviManuale() {
 ...
 const {data:matches}=await db.from('knockout').select('id,round_name,note_home,note_away,home_id,away_id,gol_home,gol_away,giocata').eq('categoria_id',STATE.activeCat);
 const risultatiKnockout={};
 for (const ko of (matches||[])) {
 const mSem=ko.round_name?.match(/SEMIFINALE\s*(\d+)/i);
