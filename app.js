// ============================================================
//  APP PRINCIPALE - Soccer Pro Experience
// ============================================================
...
function showSection(name, btn) {
  ...
  document.getElementById('cat-bar').style.display = ['a-setup'].includes(name) ? 'none' : '';
...
// Criteri: punti → scontro diretto (pts) → diff reti scontro diretto → gol fatti scontro diretto → diff reti generale → gol fatti → rigori
function calcGironeClassifica(girone) {
  ...
  // Raggruppa per parità di punti e applica spareggio
  rows.sort((a, b) => {
    if (b.pts !== a.pts) return b.pts - a.pts;
    // Scontro diretto tra i due
    const sd = getScontroDisretto(girone, a.sq.id, b.sq.id);
    if (sd !== 0) return sd; // positivo = b meglio di a
...
function getScontroDisretto(girone, idA, idB) {
  // Ritorna valore positivo se B è meglio di A nello scontro diretto
  ...
  const diffA = gfA - gfB, diffB = gfB - gfA;
...
async function renderClassifiche() {
  ...
  for (const g of gironi) {
    ...
    html += '</tbody></table>';
    // Mostra criterio spareggio se ci sono parità
    html += `<div style="font-size:11px;color:#aaa;margin-top:8px;padding-top:8px;border-top:1px solid #f5f5f5;">
    ...
    if (giocate.length) {
      ...
      for (const p of giocate) {
        ...
        if (mHome.length||mAway.length) {
          ...
          mHome.forEach(m => html += `<span class="scorer-chip">⚽ ${m.nome}${m.minuto?' '+m.minuto+"'":''} (${p.home?.nome||''})</span>`);
          mAway.forEach(m => html += `<span class="scorer-chip">⚽ ${m.nome}${m.minuto?' '+m.minuto+"'":''} (${p.away?.nome||''})</span>`);
          html += `</div>`;
        }
      }
      html += `</div>`;
...
async function renderTabellone() {
  ...
  // Raggruppa per tipo (principale / consolazione)
  const main_rounds = ko.filter(m => !m.is_consolazione);
  const cons_rounds = ko.filter(m => m.is_consolazione);

  const renderRounds = (matches) => {
    const rounds = {};
    matches.forEach(m => { if (!rounds[m.round_name]) rounds[m.round_name] = []; rounds[m.round_name].push(m); });
    const order = ['Quarti di finale','Semifinali','3° posto','Finale','5° posto','7° posto','Consolazione semifinali','Consolazione finale'];
  ...
  let html = '';
  if (main_rounds.length) {
    html += `<div class="section-label">Tabellone principale</div>`;
    html += renderRounds(main_rounds);
  }
  if (cons_rounds.length) {
    html += `<div class="section-label" style="margin-top:20px;">Consolazione</div>`;
    html += renderRounds(cons_rounds);
  }
  el.innerHTML = html;
}
...
async function renderAdminSetup() {
  ...
  <div class="card">
    <div class="form-grid-2">
      <div class="form-group"><label class="form-label">Nome torneo</label>
        <input class="form-input" id="t-nome" value="${t.nome||''}" placeholder="Soccer Pro Experience"></div>
  ...
  <div class="card">
    ...
    <div class="form-group"><label class="form-label">Squadre per girone — una riga per girone, squadre separate da virgola</label>
      <textarea class="form-input" id="cteams" rows="4" placeholder="Girone A: Juventus, Milan, Inter, Lazio
Girone B: Roma, Napoli, Fiorentina, Torino
Girone C: Atalanta, Bologna, Genoa, Lecce"></textarea></div>
...
async function renderAdminLoghi() {
  ...
  html += `<div style="font-size:13px;color:#666;margin-bottom:14px;">Clicca sul logo per caricare/cambiare l'immagine. Formati: JPG, PNG, SVG.</div>`;
  for (const sq of squadre) {
    html += `<div class="logo-team-row">
      ...
      <div style="flex:1;"><div style="font-size:14px;font-weight:600;">${sq.nome}</div>
        <div style="font-size:12px;color:#aaa;margin-top:2px;">${sq.logo?'Logo caricato':'Nessun logo'}</div></div>
      <div style="display:flex;gap:6px;">
...
async function saveRisultato(partita_id, girone_id) {
  ...
  } catch(e) { console.error('Errore:', e); toast('Errore: '+(e.message||'sconosciuto')); }
}
...
async function renderAdminKnockout() {
  ...
  const classifiche = {};
  for (const g of gironi) {
    classifiche[g.id] = calcGironeClassifica(g);
  }

  // Lista qualificate e non
  ...
  let html = `<div class="section-label">Classifica qualificate</div><div class="card" style="margin-bottom:14px;">`;
  ...
  if (consolazione.length) {
    html += `<div class="section-label">Non qualificate (consolazione)</div><div class="card" style="margin-bottom:14px;">`;
  ...
  // Sezione aggiungi partita manuale
  ...
  <div class="card" style="margin-bottom:14px;">
    <div class="form-grid-2" style="margin-bottom:10px;">
      <div><div class="form-label">Round</div>
        <select class="form-input" id="new-round-name">
          <option>Quarti di finale</option>
          <option>Semifinali</option>
          <option>3° posto</option>
          <option>Finale</option>
          <option>5° posto</option>
          <option>7° posto</option>
          <option>Consolazione semifinali</option>
          <option>Consolazione finale</option>
    ...
    <div class="form-grid-2" style="margin-bottom:10px;">
      <div><div class="form-label">Squadra 1 (casa)</div>
        <select class="form-input" id="new-home">${sqOptions}</select></div>
      <div><div class="form-label">Squadra 2 (ospite)</div>
        <select class="form-input" id="new-away">${sqOptions}</select></div>
    </div>
    <button class="btn btn-p" onclick="addKOMatch()">+ Aggiungi partita</button>
  </div>`;

  // Mostra tabellone esistente diviso in principale e consolazione
...
async function addKOMatch() {
  ...
  await dbSaveKnockoutMatch({
    ...
    round_name,
    round_order: ['Quarti di finale','Semifinali','3° posto','Finale','5° posto','7° posto','Consolazione semifinali','Consolazione finale','Consolazione 3° posto'].indexOf(round_name),
    match_order: existing.length,
    home_id,
    away_id,
...
async function resetKOSection(is_consolazione) {
  ...
  const ko = await dbGetKnockout(STATE.activeCat);
  for (const m of ko.filter(x=>x.is_consolazione===!!is_consolazione)) {
    await db.from('knockout').delete().eq('id', m.id);
  }
