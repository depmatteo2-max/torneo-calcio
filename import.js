const ROUND_META = {
 ...
 'SEMIFINALE 01': { order: 10, consolazione: false, emoji: '⚔️', desc: 'Semifinale 1' },
 'SEMIFINALE 02': { order: 11, consolazione: false, emoji: '⚔️', desc: 'Semifinale 2' },
 'SEMIFINALE 03': { order: 12, consolazione: false, emoji: '⚔️', desc: 'Semifinale 3' },
 'SEMIFINALE 04': { order: 13, consolazione: false, emoji: '⚔️', desc: 'Semifinale 4' },
 'SEMIFINALE' : { order: 10, consolazione: false, emoji: '⚔️', desc: 'Semifinale' },
 // Finali con posti (formato "FINALE 01-02° POSTO")
 'FINALE 01-02° POSTO': { order: 20, consolazione: false, emoji: '🥇', desc: 'Finale 1°-2° posto' },
 'FINALE 03-04° POSTO': { order: 21, consolazione: true, emoji: '🥉', desc: 'Finale 3°-4° posto' },
 'FINALE 05-06° POSTO': { order: 22, consolazione: true, emoji: '🏅', desc: 'Finale 5°-6° posto' },
 'FINALE 07-08° POSTO': { order: 23, consolazione: true, emoji: '⬜', desc: 'Finale 7°-8° posto' },
};
function _getRoundMeta(round) {
 const r = round.toUpperCase().trim();
 if (ROUND_META[r]) return ROUND_META[r];
 // SEMIFINALE generica con numero es. "SEMIFINALE 1" → "SEMIFINALE 01"
 const mSem = r.match(/SEMIFINALE\s*(\d+)/);
 if (mSem) {
 const key = `SEMIFINALE ${mSem[1].padStart(2,'0')}`;
 if (ROUND_META[key]) return ROUND_META[key];
 return { order: 10 + parseInt(mSem[1]) - 1, consolazione: false, emoji: '⚔️', desc: `Semifinale ${mSem[1]}` };
 }
